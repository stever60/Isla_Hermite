# HER42PB ITRAX using a previously made composite file = MSCL composite
...
HER42PB_xrf_comp <- read_csv("HER42PB/HER42PB_ITRAX_COMPOSITE_cps.csv") %>%
...
HER42PB_xrf_comp <- read_csv("Papers/Hodgson_Hermite_2022/Data/ITRAX/Outputs/HER_ITRAX_COMPOSITE_cps.csv") %>%
  filter(Site == c("HER42PB"))

HER42PB_xrf_comp

write.csv(HER42PB_xrf_comp,"HER42PB/Output/Composite/HER42PB_xrf_comp.csv", row.names = FALSE)
...
Fig3.1 <- ggplot(data = HER42PB_xrf_comp, mapping = aes(x = cps, y = `Fe a*2`)) + 
  geom_point(alpha = 0.1) + 
  theme_bw()
print(Fig3.1)

# cps - 2 std dev is too strict for HER42PB and other peat cores with a combination of low and high count matrices
cps.mean <- mean(HER42PB_xrf_comp$cps)
cps.sd <- 4*sd(HER42PB_xrf_comp$cps)
...
MSE.mean <- mean(HER42PB_xrf_comp$MSE)
MSE.sd <- 4*sd(HER42PB_xrf_comp$MSE)
...
slope1 <-  HER42PB_xrf_comp$surface - lag(HER42PB_xrf_comp$surface)
s1 <- as_tibble(slope1) %>% 
...
ggsave("HER42PB/Figures/Composite/Fig3.1_comp_tolerance_combined.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")

# Combining all 'validity' flags  ---------------------------------------------

HER42PB_xrf_comp_qc <- HER42PB_xrf_comp %>%
  mutate(slope = surface - dplyr::lag(surface)) %>%
  mutate(in_slope_tolerance = ifelse(slope <=slope.min.thres | slope >=slope.max.thres | is.na(slope) == TRUE, FALSE, TRUE)) %>%
  select(-slope) %>%
  mutate(in_cps_tolerance = ifelse(cps <=cps.min.thres | cps >=cps.max.thres | is.na(cps) == TRUE, FALSE, TRUE)) %>%
  ...
  filter(qc == TRUE) #to remove from HER42PB_xrf rows that dont pass QC

# plot summary
theme_set(theme_bw(12))
Fig3.5 <- ggplot(data = HER42PB_xrf_comp_qc, aes(y = depth, x = Ti, col = qc)) + 
...
Fig3.6 <- ggplot(data = HER42PB_xrf_comp_qc, aes(y = depth, x = Fe, col = qc)) + 
  geom_lineh(aes(group = 1)) +
  geom_rug(sides = "l", data = . %>% filter(qc == FALSE)) +
  theme(legend.title = element_text(size = 8),
        legend.text = element_text(size = 8),
...
Fig3.7 <- ggplot(data = HER42PB_xrf_comp_qc, aes(y = depth, x = Br, col = qc)) + 
  geom_lineh(aes(group = 1)) +
  geom_rug(sides = "l", data = . %>% filter(qc == FALSE)) +
  theme(legend.title = element_text(size = 8),
        legend.text = element_text(size = 8),
...
Fig3.8 <- ggplot(data = HER42PB_xrf_comp_qc, aes(y = depth, x = Mo_inc/Mo_coh, col = qc)) + 
  geom_lineh(aes(group = 1)) +
  geom_rug(sides = "l", data = . %>% filter(qc == FALSE)) +
  theme(legend.title = element_text(size = 8),
        legend.text = element_text(size = 8),
...
ggsave("HER42PB/Figures/Composite/Fig3.2_tolerance_comp_combined_examples.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")

# Remove machine elements and known matrix effect elements from dataframe ----------------
machine_elements <- select(HER42PB_xrf_comp_qc, c(Ar, Ta, W)) %>% 
  names()
machine_elements

# Create list of elements - can remove Zr (matrix effect in peat/organic seds) at this stage 
HER42PB_elements <- select(HER42PB_xrf_comp_qc, c(Mg:Mo_coh), -c(all_of(machine_elements), `Fe a*2`)) %>% # , Zr
  names()
HER42PB_elements

# REE if in dataframe
#REE <- select(HER42PB_xrf_comp_qc, c(La:Ho)) %>% 
#  names()
#REE
...
HER42PB_xrf <- HER42PB_xrf_comp %>%
  select(Site:MSE, any_of(HER42PB_elements)) %>%
  filter(validity == 1)
HER42PB_xrf
...
HER42PB_xrf <- HER42PB_xrf_comp_qc %>%
  select(Site:MSE, any_of(HER42PB_elements), qc) %>%
...
write.csv(HER42PB_xrf,"HER42PB/Output/Composite/HER42PB_xrf.csv", row.names = FALSE)
write.csv(HER42PB_xrf_stats,"HER42PB/Output/Composite/HER42PB_xrf_stats.csv", row.names = FALSE)
...
Fig3.10 <- ggarrange(
  ...
  nrow = 4, ncol = 3)
print(Fig3.10)
ggsave("HER42PB/Figures/Composite/Fig3.3_ACF_comp_pt1.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
Fig3.11 <- ggarrange(
  ...
  nrow = 4, ncol = 3)
print(Fig3.11)
ggsave("HER42PB/Figures/Composite/Fig3.4_ACF_comp_pt2.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
Fig3.12a <- apply(HER42PB_xrf %>% select(any_of(elementsList), Mo_inc, Mo_coh), 
...
print(Fig3.12a)
ggsave("HER42PB/Figures/Composite/Fig3.5a_ACF_comp_all_elements.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")

# remove machine elements and apply acf based filtering to - leaving acf elements
apply(HER42PB_xrf %>% select(any_of(elementsList), Mo_inc, Mo_coh), 2, FUN = function(x){round(Acf(x, plot = F)$acf, 3)}) %>%
  as_tibble(rownames = "lag") %>% 
  pivot_longer(!c("lag"), names_to = "elements", values_to = "value") %>% 
  mutate(lag = as.numeric(lag),
         elements = factor(elements, levels = filter(., lag == lag_thres) %>% 
...
Fig3.12b <- apply(HER42PB_xrf %>% select(any_of(acfElements)), 
  ...
  geom_dl(aes(label = elements), method = list(dl.trans(x = x + 0.5), "last.points", cex = 0.8)) # adds element labels to end
print(Fig3.12b)
ggsave("HER42PB/Figures/Composite/Fig3.5b_ACF_comp_all_elements.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
print(Fig3.13 )
ggsave("HER42PB/Figures/Composite/Fig3.6_ACF_key_elements.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig3.7_ACF_elements_depth.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
HER42PBmax100 <- select(HER42PB_xrf,Site:MSE,
                       any_of(acfElementsList) &
                         where(HER42PB_max100))
HER42PBmax100List <- select(HER42PBmax100, -c(Site:MSE, Zr)) %>%
  names() %>% 
  print()

# mean cps >50 and max cps >100
HER42PBmean50_max100 <- select(HER42PBmean50,Site:MSE,
                              any_of(acfElementsList) &
                                where(HER42PB_max100))
HER42PBmean50_max100List <- select(HER42PBmean50_max100, -c(Site:MSE, Zr)) %>%
  names() %>% 
  print()
...
write.csv(HER42PB_xrf_filter,"HER42PB/Output/Composite/HER42PB_xrf_filter.csv", row.names = FALSE)
write.csv(HER42PB_xrf_filter_stats,"HER42PB/Output/Composite/HER42PB_xrf_filter_stats.csv", row.names = FALSE)
...
HER42PB_xrf1 <- HER42PB_xrf %>%
  select(Site:MSE, label, K, Ti, any_of(HER42PB_filterList), Mo_inc, Mo_coh) %>% 
...
write.csv(HER42PB_xrf1,"HER42PB/Output/Composite/HER42PB_xrf1.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_stats,"HER42PB/Output/Composite/HER42PB_xrf1_stats.csv", row.names = FALSE)
...
HER42PB_xrf_pc_cps0 <- HER42PB_xrf_comp_qc %>%
...
write.csv(HER42PB_xrf_pc_cps_stats,"HER42PB/Output/Composite/HER42PB_xrf_pc_cps_stats.csv", row.names = FALSE)

# Filtered elements as % of cps_sum
HER42PB_xrf1_pc_cps_filtered <- select(HER42PB_xrf_pc_cps2, 
                               Site:MSE,all_of(HER42PB_elements1), cps_sum_all, cps_sum_filtered)
...
HER42PB_xrf1_pc_cps_stats <- HER42PB_xrf1_pc_cps %>%
       ...
       as_tibble(rownames="rowname")  %>%
       print()
write.csv(HER42PB_xrf1_pc_cps_stats,"HER42PB/Output/Composite/HER42PB_xrf1_pc_cps_stats.csv", row.names = FALSE)
...
write.csv(HER42PB_xrf,"HER42PB/Output/Composite/HER42PB_xrf.csv", row.names = FALSE)
write.csv(HER42PB_xrf1,"HER42PB/Output/Composite/HER42PB_xrf1.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_pc_cps,"HER42PB/Output/Composite/HER42PB_xrf1_pc_cps.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_inc,"HER42PB/Output/Composite/HER42PB_xrf1_inc.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_Ti,"HER42PB/Output/Composite/HER42PB_xrf1_Ti.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_Ln_Ti,"HER42PB/Output/Composite/HER42PB_xrf1_Ln_Ti.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_Z,"HER42PB/Output/Composite/HER42PB_xrf1_Z.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_inc_Z,"HER42PB/Output/Composite/HER42PB_xrf1_inc_Z.csv", row.names = FALSE)
write.csv(HER42PB_xrf1_Ln_Ti_Z,"HER42PB/Output/Composite/HER42PB_xrf1_Ln_Ti_Z.csv", row.names = FALSE)
...
HER42PBmean1 <- select(HER42PB_xrf1_pc_cps, Site:MSE,
                       any_of(acfElementsList) &
                         where(HER42PB_mean1))
HER42PBmean1List <- select(HER42PBmean1, -c(Site:MSE)) %>%
  names() %>% 
  print()
...
HER42PBmax1 <- select(HER42PB_xrf1_pc_cps,Site:MSE,
                      any_of(acfElementsList) &
                        where(HER42PB_max1))
HER42PBmax1List <- select(HER42PBmax1, -c(Site:MSE)) %>%
  names() %>% 
  print()

# mean %cps sum > 0.05% and max %cps sum >0.5%
HER42PBmean1_max1 <- select(HER42PBmean1,Site:MSE,
                              any_of(acfElementsList) &
                                where(HER42PB_max1))
HER42PBmean1_max1List <- select(HER42PBmean1_max1, -c(Site:MSE)) %>%
  names() %>% 
  print()
...
HER42PB_xrf2_pc_cps_filtered <- select(HER42PB_xrf_pc_cps3, 
                                      Site:MSE,all_of(HER42PB_filterList_pc), K, cps_sum_all, cps_sum_filtered, cps_sum_filtered_pc) %>% 
...
HER42PB_xrf2_pc_cps_stats <- HER42PB_xrf2_pc_cps %>%
  ...
  as_tibble(rownames="rowname")  %>%
  print()
write.csv(HER42PB_xrf2_pc_cps,"HER42PB/Output/Composite/HER42PB_xrf2_pc_cps.csv", row.names = FALSE)
write.csv(HER42PB_xrf2_pc_cps_stats,"HER42PB/Output/Composite/HER42PB_xrf2_pc_cps_stats.csv", row.names = FALSE)
...
HER42PB_xrf1_clr <- select(HER42PB_xrf1, Site:MSE, inc_coh:LnTi_Ca) %>% 
  bind_cols(HER42PB_clr1) %>%
  relocate(inc_coh:LnTi_Ca, .after = Mo_coh) %>% 
  print()

write.csv(HER42PB_xrf1_clr,"HER42PB/Output/Composite/HER42PB_xrf1_clr.csv", row.names = FALSE)
...
# used below
HER42PB_xrf <- read_csv("HER42PB/Output/Composite/HER42PB_xrf.csv")
HER42PB_xrf1 <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1.csv")
HER42PB_xrf1_pc_cps <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_pc_cps.csv")
HER42PB_xrf2_pc_cps <- read_csv("HER42PB/Output/Composite/HER42PB_xrf2_pc_cps.csv")
HER42PB_xrf1_inc_Z <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_inc_Z.csv")
HER42PB_xrf1_clr <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_clr.csv")

# not used below
#HER42PB_xrf1_inc <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_inc.csv")
#HER42PB_xrf1_Ti <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_Ti.csv")
#HER42PB_xrf1_Ln_Ti <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_Ln_Ti.csv")
#HER42PB_xrf1_Z <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_Z.csv")
#HER42PB_xrf1_Ln_Ti_Z <- read_csv("HER42PB/Output/Composite/HER42PB_xrf1_Ln_Ti_Z.csv")
...
HER42PB_elements2 <- select(HER42PB_xrf2_pc_cps, K, Ca, Ti, any_of(elementsList), Mo_inc, Mo_coh, inc_coh, coh_inc) %>% 
  names()
HER42PB_elements2
...
# used below
HER42PB_xrf_long <- select(HER42PB_xrf,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
HER42PB_xrf1_long <- select(HER42PB_xrf1,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
HER42PB_xrf1_inc_Z_long <- select(HER42PB_xrf1_inc_Z,  Site, depth, SH20_age, label, MSE, kcps, cps, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
HER42PB_xrf1_pc_cps_long <- select(HER42PB_xrf1_pc_cps,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
HER42PB_xrf2_pc_cps_long <- select(HER42PB_xrf2_pc_cps,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
HER42PB_xrf1_clr_long <- select(HER42PB_xrf1_clr,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")

# not used below
#HER42PB_xrf1_inc_long <- select(HER42PB_xrf1_inc,  Site, depth, SH20_age, label, any_of(HER42PB_elements2)) %>%
#  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
#HER42PB_xrf1_Z_long <- select(HER42PB_xrf1_Z,  Site, depth, SH20_age, label, MSE, kcps, cps, any_of(HER42PB_elements2)) %>%
#  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
#HER42PB_xrf1_Ln_Ti_Z_long <- select(HER42PB_xrf1_Ln_Ti_Z,  Site, depth, SH20_age, label, MSE, kcps, cps, any_of(HER42PB_elements2)) %>%
#  pivot_longer(any_of(HER42PB_elements2), names_to = "param", values_to = "element")
...
ggsave("HER42PB/Figures/Composite/Fig4.1a_Corr_matrix_xrf1_cps.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig4.1b_Corr_matrix_xrf1_cps.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggcorr(HER42PB_xrf1_inc_Z[,HER42PB_elements2], method = c("everything", "pearson"), 
       size = 3, label = TRUE, label_alpha = TRUE, label_round=2, label_size = 3)
ggsave("HER42PB/Figures/Composite/Fig4.3_Corr_matrix_incZ.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggcorr(HER42PB_xrf1_pc_cps[,HER42PB_elements2], method = c("everything", "pearson"), 
       size = 3, label = TRUE, label_alpha = TRUE, label_round=2, label_size = 3)
ggsave("HER42PB/Figures/Composite/Fig4.2_Corr_matrix_xrf1_pc_cps_sum.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")

# clr - filtered elements2 
ggcorr(HER42PB_xrf1_clr[,HER42PB_elements2], method = c("everything", "pearson"), 
       size = 3, label = TRUE, label_alpha = TRUE, label_round=2, label_size = 3) 
ggsave("HER42PB/Figures/Composite/Fig4.4_Corr_matrix_xrf1_clr.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig4.5_Corr-den_matrix_xrf1_cps.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig4.7_Corr-den_matrix_xrf1_Z.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig4.6_Corr-den_matrix_xrf1_pc_cps_sum.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
ggsave("HER42PB/Figures/Composite/Fig4.8_Corr-den_matrix_xrf1_clr.pdf", 
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
HER42PB_xrfStrat_cps <- HER42PB_xrf1_long  %>%
  ...
  facet_geochem_gridh(vars(param)) +
  labs(x = "Peak area [cps]", y = "Depth [cm]") +
  ggtitle("HER42PB-ITRAX: cps (filtered elements)")
...
HER42PB_xrfStrat_inc_Z <- HER42PB_xrf1_inc_Z_long  %>% 
  ...
  ggtitle("HER42PB-ITRAX: cps/inc as Z-scores (filtered elements)")
...
HER42PB_xrfStrat_pc_cps <- HER42PB_xrf1_pc_cps_long  %>% 
  ...
  ggtitle("HER42PB-ITRAX: %cps sum (filtered elements)")
...
HER42PB_xrfStrat_clr <- HER42PB_xrf1_clr_long  %>% 
  ...
  ggtitle("HER42PB-ITRAX: clr - centred log ratio (filtered elements)")

# Summary plots
ggarrange(HER42PB_xrfStrat_cps, HER42PB_xrfStrat_inc_Z, nrow = 2)
ggsave("HER42PB/Figures/Composite/Fig5.1_Summary_cps_incZ_depth_plots.pdf",
       height = c(30), width = c(30), dpi = 600, units = "cm")

ggarrange(HER42PB_xrfStrat_pc_cps, HER42PB_xrfStrat_clr, nrow = 2)
ggsave("HER42PB/Figures/Composite/Fig5.2_Summary_cpssum_clr_depth_plots.pdf",
       height = c(30), width = c(30), dpi = 600, units = "cm")
...
HER42PB_xrfStrat_pc_cps_CONISS <- HER42PB_xrfStrat_pc_cps +
  ...
  ggtitle("HER42PB-ITRAX: %cps sum (filtered elements) with CONISS")
...
ggsave("HER42PB/Figures/Composite/Fig5.3_Summary_cpssum_clr_CONISS_depth_plots.pdf",
       height = c(30), width = c(30), dpi = 600, units = "cm")

# Summary plots vs age --------------------------------------------------------

# cps
HER42PB_xrfAge_cps <- HER42PB_xrf1_long  %>%
  filter(param %in% HER42PB_elements2) %>%
  mutate(param = fct_relevel(param, HER42PB_elements2)) %>%
...
# cps/cin as Z-scores
HER42PB_xrfAge_inc_Z <- HER42PB_xrf1_inc_Z_long  %>% 
  filter(param %in% HER42PB_elements2) %>%
  mutate(param = fct_relevel(param, HER42PB_elements2)) %>%
  ggplot(aes(x = SH20_age, y = element)) +
  #geom_point(size = 0.01) +
...
# %cps sum
HER42PB_xrfAge_pc_cps <- HER42PB_xrf1_pc_cps_long  %>% 
  filter(param %in% HER42PB_elements2) %>%
  mutate(param = fct_relevel(param, HER42PB_elements2)) %>%
  ggplot(aes(x = SH20_age, y = element)) +
  #geom_point(size = 0.01) +
...
# clr
HER42PB_xrfAge_clr <- HER42PB_xrf1_clr_long  %>% 
  filter(param %in% HER42PB_elements2) %>%
  mutate(param = fct_relevel(param, HER42PB_elements2)) %>%
  ggplot(aes(x = SH20_age, y = element)) +
  #geom_point(size = 0.01) +
...
# Summary plots
ggarrange(HER42PB_xrfAge_cps, HER42PB_xrfAge_inc_Z, ncol = 2)
ggsave("HER42PB/Figures/Composite/Fig5.4_Summary_cps_incZ_age_plots.pdf",
       height = c(30), width = c(30), dpi = 600, units = "cm")

ggarrange(HER42PB_xrfAge_pc_cps, HER42PB_xrfAge_clr, ncol = 2)
ggsave("HER42PB/Figures/Composite/Fig5.5_Summary_cpssum_clr_age_plots.pdf",
       height = c(30), width = c(30), dpi = 600, units = "cm")
